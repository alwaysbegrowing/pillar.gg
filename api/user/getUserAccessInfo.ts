...
getClipClockUserInfo is called to verify user privleges across the web app.
...
information is returned in compliance with <API.ClipClockUser> interface
...
const getUserAccessInfo = async (req: VercelRequest, res: VercelResponse) => {
  try {
    ...
    // look up user
    // eslint-disable-next-line
    const user_result = await db.collection('users').findOne( {"twitch_credentials.access_token" : req.body.twitch_access_token });
